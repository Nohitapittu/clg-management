...
def admin_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('finalapp:dashboard')  # Change the redirection URL to your desired home page
        else:
            messages.info(request, 'Invalid user credentials')
            return redirect('finalapp:admin_login')
    else:
...
def admin_register(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            ...
            return redirect('finalapp:admin_register')
...
def student_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            auth.login(request, user)
            return redirect('finalapp:home')  # Change the redirection URL to your desired home page
        else:
            messages.info(request, 'Invalid user credentials')
...
def send_email(request):
    ...
    from_email = request.POST.get("from_email", "mohammedrabiyanikhat@gmail.com")
    if subject and message and from_email:
        try:
            send_mail(subject, message, from_email, ["mohammedrabiyanikhat@gmail.com"])
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                ...
                return redirect('account_login')  # Adjust the URL to your login page
            question_paper.save()
            return redirect('finalapp:success')
    else:
...
def update_fee(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            fee.save()
            return redirect('finalapp:view_fees')
    else:
        form = FeeUpdateForm()
...
def take_attendance(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            attendance.save()
            return redirect('finalapp:admin_attendance')
    else:
        form = AttendanceForm()
...
def overview(request):
    total_teachers = 16  # Replace with actual data
    total_students = 24  # Replace with actual data
    students = [{'Firstname': 'MSDS'}, {'Firstname': 'MSCS'}, {'Firstname': 'MECS'},{'Firstname': 'MPCS'},{'Firstname': 'MCCS'},{'Firstname': 'MSCA'},{'Firstname': 'BZC'},{'Firstname': 'AZC'},{'Firstname': 'FMB'},{'Firstname': 'FMC'},{'Firstname': 'BCOM'},{'Firstname': 'BBA'},]
    present_days = [9.9,8.5,9.2,8.5,9.7,8.6,9.5,7.9,8.8,9.1,9.4,8.8]  # Replace with actual data
...
from django.shortcuts import render, redirect
from finalapp.forms import AskDateForm, AttendanceForm
from finalapp.models import Attendance, Student
...
def admin_take_attendance_view(request, Co):
    students = Student.objects.filter(Co=Co)
    attendance_data = []
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            selected_date = form.cleaned_data['attendance_date']
    ...
    for student in students:
        # Retrieve attendance instances for each student on the selected date
        student_attendance = Attendance.objects.filter(student=student, date=selected_date)

        # Add relevant data to the attendance_data list
    ...
    return render(request, 'admin_take_attendance.html', context)
...
def enter_attendance(request):
    ...
    students = Student.objects.filter(Co='MSDS')  # Replace 'MSDS' with the desired course
...
def view_attendance(request):
    ...
    # You can pass the attendance records to the template context
...
def get_student_by_id(student_id):
    # Implement your logic to get a student by ID from the database
    # This could be a simple query like Student.objects.get(id=student_id)
    pass
...
def admin_view_attendance_view(request, student):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Get the student object based on the provided name or code
            student_obj = get_object_or_404(Student, Co=student)  # Replace 'name' with the actual field you're filtering on

            # Filter Attendance objects based on the 'date' and 'student' (Student object)
            attendancedata = Attendance.objects.filter(date=date, student=student_obj)

            # Zip the Attendance and Student data for rendering in the template
            mylist = zip(attendancedata, [student_obj])  # Using a list to match the structure of zip

            return render(request, 'admin_view_attendance_page.html', {'student': student_obj, 'mylist': mylist, 'date': date})
        else:
            print('form invalid')

    return render(request, 'admin_view_attendance_ask_date.html', {'student': student, 'form': form})
